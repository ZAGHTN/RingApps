...
func copy cStr,nCount 
	# Le programme fonctionne, mais on a élliminé 
	# la fonction standard copy en la remplassant 
	# par la fonction copy crée récément.
	for t=1 to nCount
		? cStr
	next
...
class myClass
	func copy cStr,nCount 
	func test
		//copy("ring",3) # methode de myClass
		new local{
			copy("ring2",3) # fonction hors classe
		}
